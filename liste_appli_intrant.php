...
try{
    // Récupération des applications d'intrants pour l'utilisateur connecté
    $sql ="SELECT 
                a.id, 
                a.date_aplication,
           ...
           AND a.id_agriculteur = :id_agriculteur 
           ORDER BY a.date_aplication DESC"; 
    ...
    $error_message = "Erreur de récupération des applications d'intrants : " . $e->getMessage();
}
?>
...
<head> 
    ...
    <title>Liste des Applications d'Intrants | MonAgriCoach</title> 
    ...
    <style>
        /* --- PALETTE HARMONISÉE (Émeraude DOMINANTE, Or pour Déconnexion) --- */
        :root {
            --color-primary-emerald: #06bdbdff; 
            --color-primary-dark: #0cb4b4ff; 
            
            /* Rétablissement de l'Or pour le bouton de déconnexion */
            ...
            --color-disconnect-bg: var(--color-secondary-gold); /* Reconnexion du bouton à l'Or */
        ...
        /* Lien Actif : Application Intrant */
        ...
        /* --- STYLE DES LISTES D'APPLICATIONS --- */
...
<body> 
    ...
    <div class="main-content">
        <div class="header">
            <h1>Gestion de la Fertilisation</h1>
            <a href="appli_intrant.php" class="add-new-link">
                <i class="fas fa-plus-circle"></i> Ajouter un engrais
            </a> 
        </div>
        ...
        <div class="content-container">
            <h2>Gérer l'ensemble de vos Fertilisation</h2>
            ...
            <form action="recherche_appli_intrant.php" method="GET" class="search-form">
                ...
                <button type="button" onclick="window.location.href='liste_appli_intrant.php'"><i class="fas fa-sync-alt"></i> Réinitialiser</button>
            </form>

            <?php if (count($apintrants) > 0): ?>
                <ul class="appli-list">
                    <?php foreach($apintrants as $apintrant ): 
                        // Logique pour déterminer la classe de statut (démo/affichage)
                        $statut = htmlspecialchars($apintrant['statut_plantation']);
                        $status_class = 'status-growing'; // Défaut
    ...
    <span>
        <?= htmlspecialchars($apintrant['quantite_appliquee']); ?> 
        <?= htmlspecialchars($apintrant['unite_appliquee'] ?? ''); ?>
    </span>
</div>
                                        <div class="metadata-detail">
            ...
            <?php else:?> 
                <p class="no-records"><i class="fas fa-info-circle"></i> Aucune application d'intrant enregistrée. Cliquez sur "**Ajouter une Application**" pour commencer.</p> 
            <?php endif;?>
        </div> 
    </div>
</body>
