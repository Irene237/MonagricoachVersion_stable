...
$error_message = null;
$conditions = [];
$params = [':userId' => $userId]; // L'ID utilisateur est toujours un paramètre initial

// --- 1. Récupération du terme de Recherche (GET) ---
// Utilisation de 'recherche' (du formulaire) et sécurisation
$termeRecherche = $_GET['recherche'] ?? ''; 

// Construction de la requête de base
$query_string ="SELECT id, nom_intrant, type_intrant, unite_standard, descriptions 
                FROM intrant 
                WHERE id_agriculteur = :userId";

// Ajout des conditions de recherche si un terme est présent
if (!empty($termeRecherche)) {
    $search_pattern = '%' . $termeRecherche . '%';
    // Ajout d'une condition pour rechercher dans le nom ou la description
    $query_string .= " AND (nom_intrant LIKE :termeRecherche OR descriptions LIKE :termeRecherche)";
    $params[':termeRecherche'] = $search_pattern;
}

// Ajout de l'ORDER BY
$query_string .= " ORDER BY nom_intrant ASC";
...
try{
    ...
    error_log("Erreur de recherche des intrants: " . $e->getMessage());
    $error_message = "Erreur de récupération des intrants : Une erreur technique est survenue.";
}
...
<head> 
    ...
    <title>Résultats de Recherche Intrants | MonAgriCoach</title> 
    ...
    <style>
        ...
        /* Lien Actif : Intrant */
        .sidebar li a[href*="intrant.php"] { /* Utiliser * pour cibler liste et recherche */
            background-color: rgba(6, 189, 189, 0.1); 
            color: var(--color-primary-emerald); 
            font-weight: 600;
            border-left: 5px solid var(--color-primary-emerald); 
        }
        .sidebar li a[href*="intrant.php"] i {
             color: var(--color-primary-emerald); 
        }
        ...
        /* --- STYLE DES LISTES D'INTRANTS (Format Carte) --- */
...
<body> 
    ...
    <div class="main-content">
        <div class="header">
            <h1>Résultats de Recherche Intrants</h1>
        ...
        <div class="content-container">
            <h2>Résultats pour "<?= htmlspecialchars($termeRecherche); ?>"</h2>
            
            <?php if (!empty($termeRecherche)): ?>
                <div class="search-info">
                    <i class="fas fa-filter"></i> Affichage filtré. Terme : **<?= htmlspecialchars($termeRecherche); ?>**.
                    <a href="liste_intrant.php" style="color: var(--color-primary-dark); font-weight: 700; text-decoration: none; margin-left: auto;">Voir tous les intrants.</a>
                </div>
            <?php endif; ?>
            ...
            <form action="recherche_intrant.php" method="GET" class="search-form">
                ...
                <input type="text" name="recherche" placeholder="Recherche par nom d'intrant..." id="search_query" value="<?= htmlspecialchars($termeRecherche); ?>">
            ...
            <?php else:?> 
                <p><i class="fas fa-search"></i> Aucun intrant trouvé pour le terme: "**<?= htmlspecialchars($termeRecherche); ?>**".</p> 
            <?php endif;?>
        </div> 
    </div>
</body>
