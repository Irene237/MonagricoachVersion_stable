...
// --- Bloc de Vérification de Connexion (Essentiel pour la sécurité) ---
...
$error_message = null;
$conditions = [];
$params = [':userId' => $userId]; // L'ID utilisateur est toujours un paramètre

// Début de la requête SQL (limitée à l'agriculteur connecté)
$query_string = "SELECT 
                    c.id, 
                    c.nom_commun, 
...
// --- 1. Gestion de la Recherche par Terme (Nom ou Besoins Eau) ---
if (!empty($_GET['recherche'])) {
    $termeRecherche = '%' . $_GET['recherche'] . '%';
    // Ajout d'une condition pour le nom ou les besoins en eau
    $conditions[] = "(c.nom_commun LIKE :termeRecherche OR c.besoins_eau_mm LIKE :termeRecherche)";
    $params[':termeRecherche'] = $termeRecherche;
}

// --- 2. Gestion du filtre par Cycle de Vie (Conservé de votre code d'origine) ---
if (!empty($_GET['cycle_de_vie_jours'])) {
    $conditions[] = "c.cycle_de_vie_jours = :cycle_de_vie_jours";
    $params[':cycle_de_vie_jours'] = $_GET['cycle_de_vie_jours'];
}

// Construction de la clause WHERE finale
if (!empty($conditions)) {
    // Si des conditions de recherche existent, nous les ajoutons à la clause WHERE existante (u.id = :userId)
    $query_string .= " AND " . implode(" AND ", $conditions);
}

// Ajout de l'ORDER BY (harmonisation avec liste_culture.php)
$query_string .= " ORDER BY c.nom_commun ASC";
...
try{
    ...
    error_log("Erreur de recherche des cultures: " . $e->getMessage());
    $error_message = "Erreur de récupération des cultures : Une erreur technique est survenue.";
}
?>
...
<head> 
    ...
    <title>Résultats de Recherche Cultures | MonAgriCoach</title> 
    ...
    <style>
        /* --- PALETTE HARMONISÉE (Copie conforme de liste_culture.php) --- */
        :root {
        ...
        /* Message d'info Recherche */
        .search-info {
            padding: 15px 20px;
            background-color: #FFFBEA; /* Jaune très clair */
        ...
        /* --- STYLE DES CARTES DE CULTURE (Copie conforme de liste_culture.php) --- */
        ...
        .card-header-culture {
            ...
            justify-content: space-between;
            align-items: center;
            border-bottom: 2px solid var(--color-primary-emerald); 
            padding-bottom: 15px;
            margin-bottom: 20px;
        }
        ...
        /* Correction pour les icônes de la sidebar (étaient des emojis dans l'ancien code) */
        .sidebar li a i {
             /* Conserve le style Fa-Font pour l'harmonisation */
...
<body> 
    ...
    <div class="main-content">
        <div class="header">
            <h1>Résultats de Recherche Cultures</h1>
        ...
        <div class="content-container">
            ...
            <?php if (!empty($_GET['recherche']) || !empty($_GET['cycle_de_vie_jours'])): ?>
                <div class="search-info">
                    <i class="fas fa-filter"></i> Affichage filtré. <a href="liste_culture.php" style="color: var(--color-primary-dark); font-weight: 700;">Voir toutes les cultures.</a>
                </div>
            <?php endif; ?>
            ...
            <form action="recherche_culture.php" method="GET" class="search-form">
                ...
                <input type="text" name="recherche" placeholder="Recherche par nom ou besoin en eau..." id="search_query" value="<?= htmlspecialchars($_GET['recherche'] ?? ''); ?>">
            ...
            <?php else: ?> 
                <p><i class="fas fa-info-circle"></i> Aucune culture ne correspond à vos critères de recherche.</p> 
            <?php endif; ?>
        </div> 
    </div>
</body>
