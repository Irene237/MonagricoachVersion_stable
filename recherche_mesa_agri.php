...
$cons = [];
$errorMessage = '';
$terme_recherche = $_GET['recherche'] ?? ''; // Terme de recherche
$date_fin_assistance = $_GET['date_fin_assistance'] ?? ''; // Filtre de date (non utilisé dans le formulaire visible, mais conservé pour le PHP)


// Requête de base pour obtenir l'historique complet des messages pour l'agriculteur connecté.
$sql = "SELECT 
            m.id, 
...
$params = [':id_agriculteur_assiste' => $id_agriculteur_assiste];
$conditions = [];

// --- LOGIQUE DE RECHERCHE & FILTRES ---

// 1. Recherche par mot-clé (nom conseiller, message conseiller/agriculteur)
if (!empty($terme_recherche)) {
    $termeRechercheLike = '%' . strtolower($terme_recherche) . '%';
    // Utilisation de LOWER() pour une recherche insensible à la casse
    $conditions[] = "(
                        LOWER(u.nom) LIKE :termeRecherche 
                        OR LOWER(u.prenom) LIKE :termeRecherche 
                        OR LOWER(m.message_agriculteur) LIKE :termeRecherche
                        OR LOWER(m.message_conseiller) LIKE :termeRecherche
                     )";
    $params[':termeRecherche'] = $termeRechercheLike;
}

// 2. Filtre par date de fin d'assistance (si spécifié)
...
// Tri des résultats
$sql .= " ORDER BY m.date_debut_assistance DESC";
...
try {
    ...
    // Gestion d'erreur
    error_log("Erreur de récupération des conseils lors de la recherche: " . $e->getMessage());
...
<head>
    ...
    <title>Résultats de Recherche | Mes Conseils | MonAgriCoach</title>
    ...
    <style>
        /* ----------------------------------------------------- */
        /* --- STYLES GLOBALS & PALETTE (Copie conforme) --- */
        /* ----------------------------------------------------- */
        :root {
        ...
        /* Bouton Déconnexion (Adaptation du style de liste_message_agri.php) */
        .disconnect-item {
            margin-top: auto; 
            padding: 25px; 
        }
...
<div class="main-content">
    <div class="header">
        <h1>Résultats de Recherche de Demandes de conseil</h1>
    ...
    <div class="content-container">
        ...
        <form action="recherche_mesa_agri.php" method="GET" class="search-form">
            ...
            <input type="text" name="recherche" placeholder="Rechercher par nom du conseiller ou mot-clé..." id="search_query" value="<?= htmlspecialchars($terme_recherche); ?>">
            <button type="submit"><i class="fas fa-search"></i> Rechercher</button>
            <button type="button" onclick="window.location.href='liste_message_agri.php'"><i class="fas fa-sync-alt"></i> Liste Complète</button>
        </form>

        <p class="search-info">
            <i class="fas fa-filter"></i> **<?= count($cons); ?>** message(s) trouvé(s) pour le critère : **"<?= htmlspecialchars($terme_recherche); ?>"**.
        </p>
                ...
                <?php foreach($cons as $con ): ?>
                    <div class="conseil-card">
                        ...
                        <div class="card-header">
                            ...
                            <p>
                                **Conseiller:** <?= htmlspecialchars($con['prenom_conseiller'] . ' ' . $con['nom_conseiller']); ?><br>
                                **Date Demande:** <?= date('d/m/Y', strtotime($con['date_debut_assistance'])); ?>
                            </p>
                        </div>
                        ...
                        <div class="agriculteur-message">
                            ...
                            <p>
                                <?= nl2br(htmlspecialchars(truncate_message($con['message_agriculteur'] ?? 'N/A'))); ?>
                            </p>
                        </div>
                        ...
                        <div class="card-footer">
                            <div class="card-actions">
                                <a href="reponse.php?id=<?= $con['id']; ?>" title="Consulter la conversation" class="reply-button">
        ...
        <?php else:?> 
            <p class="no-data-message"><i class="fas fa-info-circle"></i> **Aucun message trouvé** pour votre recherche : **"<?= htmlspecialchars($terme_recherche); ?>"**.</p> 
        <?php endif;?>
